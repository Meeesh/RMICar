...
public class Core implements Runnable{
     ...
     */
    private  boolean bGameFinishing = false;
   ...
   // private GUI gGUI;
    ...
    private static CoreServer coreserv = null;
                ...
                try
                {
                    ...
                    if(gameRunTime%gameMaxRunTime == 0 && isbGameInProgress() == true)
                    {
                       for(int c = 0; c<clientList.size();c++){    
                        ...
                        setbGameFinishing(manageCollisions(vCars, vTabObstacles, isbGameFinishing(), c ));
   ...
   /* */                    moveCars(isUP_P((int)clientList.get(c)), isDO_P((int)clientList.get(c)), isLE_P((int)clientList.get(c)), isRI_P((int)clientList.get(c)), vCars, (int)clientList.get(c));
                    ...
                    if(!isbGameFinishing())
                    {
                        ...
                        htIdClient.get(clientList.get(c)).setPosiP(posP);
                        //System.out.println("Id client : " + c + "CliPos : " + htIdClient.get(clientList.get(c)).getPosi() + '\n');
                ...
                if(rInter != null)
                {
                    ...
                    if(currentObstacle.effect == 3 && myCar.id == 6)
                    {
                        ...
                        if(!bgf)
                        {
                            ...
                            System.out.println("IDClient : "+ id +" -> POS : "+ arri);
                            if(arri == 1)
                            {
...
/**/                                 
    ...
    public void newGrid()
    {
        ...
        setiNbParticipants(1);
        while(iCars.hasNext())
        {
            Car currentCar = iCars.next();
            if(currentCar.Racer)
            {
                setiNbParticipants(getiNbParticipants() + 1);
    ...
    public void notifyclient(String mess) throws java.rmi.RemoteException {
        
       for (int i = 0; i < clientList.size(); i++){
      System.out.println("doing "+ i +"-th callback\n");    
      // convert the vector object to a callback object
       int clientid = 
        ( int)clientList.elementAt(i);
      // invoke the callback method
     
         coreserv.notifyclient(clientid, mess);
    }// end for
 ...
 public void setPlayerButton(boolean t) throws java.rmi.RemoteException
   {
       for (int i = 0; i < clientList.size(); i++){
      System.out.println("doing "+ i +"-th callback\n");    
      // convert the vector object to a callback object
...
// the server to uniquely identify the registered client.
  ...
  private synchronized void doCallbacks(int id ) throws java.rmi.RemoteException {
   ...
   /* System.out.println(
    ...
    coreserv.update (id, vDisplayRoad, vDisplayObstacles, vDisplayCars, htIdClient.get(id).getCar(), getiFinalPosition(), getiNbParticipants(), isbGameFinishing(), htIdClient.get(id).getPosi(),htIdClient.get(id).getPosiP());
  ...
  //}// end for
    ...
    /**
     * @param sFinalPosition the sFinalPosition to set
     */
    ...
    /**
     * @param iFinalPosition the iFinalPosition to set
     */
    ...
    /**
     * @return the iNbParticipants
     */
    public int getiNbParticipants() {
        return iNbParticipants;
    }

    /**
     * @param iNbParticipants the iNbParticipants to set
     */
    public void setiNbParticipants(int iNbParticipants) {
        this.iNbParticipants = iNbParticipants;
    }
    ...
    /**
     * @param abGameFinishing the bGameFinishing to set
     */
    public void setbGameFinishing(boolean abGameFinishing) {
        bGameFinishing = abGameFinishing;
    }

    /**
     * @return the bGameInProgress
     */
    ...
    /**
     * @param abGameInProgress the bGameInProgress to set
     */
    public void setbGameInProgress(boolean abGameInProgress) {
        bGameInProgress = abGameInProgress;
    }

    /**
     * @return the bGameQuit
     */
    ...
    /**
     * @param abGameQuit the bGameQuit to set
     */
    public void setbGameQuit(boolean abGameQuit) {
        bGameQuit = abGameQuit;
    }
